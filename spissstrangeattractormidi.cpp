...
// spissstrangeattractormidi.cpp : Defines the entry point for the console application.
  ...
  case WM_CREATE:
	  {
		...
		//SetTimer( hWnd, global_miditimer, 10, NULL );
		//SetTimer( hWnd, global_miditimer, 10000, NULL );
		//SetTimer( hWnd, global_miditimer, 5000, NULL );
		//SetTimer( hWnd, global_miditimer, 2500, NULL );
		//SetTimer( hWnd, global_miditimer, 1000, NULL ); //original
		//SetTimer( hWnd, global_miditimer, 100, NULL );
		//SetTimer( hWnd, global_miditimer, 400, NULL );
		//SetTimer( hWnd, global_miditimer, 8000, NULL ); //dj-oifii
		SetTimer( hWnd, global_miditimer, 4000, NULL ); //dj-oifii
		//SetTimer( hWnd, global_miditimer, 2000, NULL ); //dj-oifii
		//SetTimer( hWnd, global_miditimer, 500, NULL ); //dj-oifii
		//SetTimer( hWnd, global_miditimer, 250, NULL ); //dj-oifii
		//SetTimer( hWnd, global_miditimer, 40, NULL );
		//SetTimer( hWnd, global_miditimer, 20, NULL );
		//SetTimer( hWnd, global_miditimer, 10, NULL );


		SetTimer( hWnd, global_miditimer_programchange,3600000, NULL ); //dj-oifii
		//SetTimer( hWnd, global_miditimer_programchange,180000, NULL ); //dj-oifii
		//SetTimer( hWnd, global_miditimer_programchange, 5000, NULL );
		//SetTimer( hWnd, global_miditimer_programchange, 30000, NULL ); //dj-oifii
...
void OnTimer(HDC hDC) //increment and display
{
		...
		*/
		...
		if(global_bdrawlabel && global_bdrawpoint)
		{
			glEnable(GL_TEXTURE_2D);
			//glColor4f(1.0f, 0.0f, 0.0f, 1.0f); //red
			//glColor4f(0.0f, 0.0f, 1.0f, 1.0f); //blue
		...
		if(global_bsendmidi && global_bdrawpoint)
		{
			PmEvent myPmEvent;
...
void GetConfig()
{
        ...
        if (RegOpenKeyEx( HKEY_CURRENT_USER,
                ...
                L"Software\\Oifii\\Spissstrangeattractormidi", //lpctstr
                0,                      //reserved
                KEY_QUERY_VALUE,
                &key) == ERROR_SUCCESS) 
        {
